...
def reformat_receiver(data):
    for c in range(len(data['receiver'])):
        receiver_Kegg_ID = data['receiver Kegg ID'][c]
        ...
        if receiver_last_space_index != -1:
            ...
            receiver_append(receiver_Kegg_ID, c)
...
def get_receiver_alias(id, k):
    receiver_alias = data['receiver alias'][k]
    try:
...
def receiver_append(id, k):
    ...
    receiver_type_list.append(data['receiver type'][k])

    starter.append(data['starter'][k])
    starter_type_list.append(data['starter type'][k])
    starter_link_list.append(data['starter link'][k])
    starter_kegg_IDs.append(data['starter Kegg ID'][k])
    starter_alias_list.append(data['starter alias'][k])

    relation_name.append(data['relation name'][k])
    relation_type.append(data['relation type'][k])
    source.append(data['source'][k])
...
def write_to_csv():
    ...
    gene_relation_df.to_csv('pathway_v3.csv', index=False)
...
if __name__ == '__main__':
    ...
    data = pd.read_csv('pathway_v3_starter.csv')
    reformat_receiver(data)
    reformat_alias()
    write_to_csv()
