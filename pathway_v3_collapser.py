...
def collapse_v3(csv_data):
    for i in range(len(csv_data['starter'])):
        repeated = False
        for j in range(len(starter_kegg_Names)):
            if csv_data['starter Kegg ID'][i] == starter_kegg_Names[j] and csv_data['receiver Kegg ID'][i] == receiver_kegg_Names[j]:
                count[j] += 1
                singleton[j] = False
                repeated = True
                source[j] += ',\n ' + csv_data['source'][i]
        ...
        if repeated == False:
            starter.append(csv_data['starter'][i])
            starter_type_list.append(csv_data['starter type'][i])
            starter_link_list.append(csv_data['starter link'][i])
            starter_kegg_Names.append(csv_data['starter Kegg ID'][i])
            starter_alias_list.append(csv_data['starter alias'][i])
            receiver.append(csv_data['receiver'][i])
            receiver_type_list.append(csv_data['receiver type'][i])
            receiver_link_list.append(csv_data['receiver link'][i])
            receiver_kegg_Names.append(csv_data['receiver Kegg ID'][i])
            receiver_alias_list.append(csv_data['receiver alias'][i])
            relation_name.append(csv_data['relation name'][i])
            relation_type.append(csv_data['relation type'][i])
            source.append(csv_data['source'][i])
            count.append(1)
            singleton.append(True)

        if i % 1000 == 0:
            print(i / len(csv_data['starter']))

    gene_relation_dict = {
        'starter': starter,
        'starter_type': starter_type_list,
        'starter_link': starter_link_list,
        'starter_Kegg_ID': starter_kegg_Names,
        'starter_alias': starter_alias_list,
        'receiver': receiver,
        'receiver_type': receiver_type_list,
        'receiver_link': receiver_link_list,
        'receiver_Kegg_ID': receiver_kegg_Names,
        'receiver_alias': receiver_alias_list,
    ...
    gene_relation_df = pd.DataFrame(gene_relation_dict)
    gene_relation_df.to_csv('pathway_v3_collapsed.csv', index=False)
    ...
    for i in range(length):
        ...
        try:
            ...
            for a in range(len(csv_data['starter'])):
                repeated = False
                for j in range(len(starter_kegg_Names)):
                    if csv_data['starter Kegg ID'][a] == starter_kegg_Names[j] and csv_data['receiver Kegg ID'][a] == \
                            receiver_kegg_Names[j] and csv_data['starter'][a] == starter[j] and csv_data['receiver'][a]\
                            == receiver[j] and csv_data['starter link'][a] == starter_link_list[j] and csv_data['receiver link'][a] == receiver_link_list[j]\
                            and csv_data['relationship'][a] == relation_name[j] and csv_data['relation type'][a] == relation_type[j]:
                        count[j] += 1
                        singleton[j] = False
                        repeated = True
                        source[j] += ',\n ' + pathway_list[i] + ' ' + title_list[i]
                        break
                    else:
                        continue
...
if __name__ == '__main__':
    ...
    csv_data = pd.read_csv('pathway_v3.csv')
    collapse_v3(csv_data)